...
gameovertext = font.render("GAME OVER", 1, c.WHITE)
starttext = font.render("PRESS ANY KEY TO START", 1, c.WHITE)
clock = pg.time.Clock()
...
def notLimit(pos, move):
    if ((move == UP and pos[1] - 1 != 0)
        or(move == DOWN and pos[1] + 1 != HEIGHT - 1)
        or (move == LEFT and pos[0] - 1 != 0)
            or (move == RIGHT and pos[0] + 1 != WIDTH - 1)):
        return True
    else:
        return False
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT:
            ...
            if event.key == pg.K_ESCAPE:
                sys.exit()
            elif event.key == pg.K_UP and direction != UP:
                move = UP
            elif event.key == pg.K_DOWN and direction != DOWN:
                move = DOWN
            elif event.key == pg.K_RIGHT and direction != RIGHT:
                move = RIGHT
            elif event.key == pg.K_LEFT and direction != LEFT:
                move = LEFT
...
while True:
    window.blit(gameovertext, ((WIDTH - WIDTH / 4) *
                               BS / 2, (HEIGHT - HEIGHT / 10) * BS / 2))
    pg.display.flip()
    event = pg.event.wait()
    if event.type == pg.QUIT:
        sys.exit()
